...
)
{
	...
	vsnprintf_s(buff, 1024, _TRUNCATE, msg, args);
...
std::string Dbg::basename(
	const std::string& file
)
{
	unsigned int i = (unsigned int)file.find_last_of("\\/");
...
vector<StackFrame> Dbg::stack_trace()
{
#if _WIN64
...
)
{
	...
	for (unsigned int i = 0; i < stack.size(); i++)
	{
		buff << "0x" << std::hex << stack[i].address << ": " << stack[i].name << "(" << stack[i].line << ") in " << stack[i
		].module << "\n";
	}
